<script lang="ts">
  ...
  import {hasColdIssue, hasFailSafeIssue, hasHotIssue, hasMixedIssue} from "../../lib/helpers/conditionals/hasHotIssue";
  ...
  async function submit() {
    [results.hot, results.cold, results.mixed, results.failSafe] = await Promise.all([
    ...
    <div class="space-y-8 divide-y divide-gray-200 sm:space-y-5">
        <div class="space-y-6 pt-8 sm:space-y-5 sm:pt-10">
            ...
            <div class="space-y-6 sm:space-y-5">
                <TemperatureInput id="hot-result" name="Hot" bind:value={results.hot.temperature}/>
                <TemperatureInput id="cold-result" name="Cold" bind:value={results.cold.temperature}/>
                <TemperatureInput id="mixed-result" name="Mixed" bind:value={results.mixed.temperature}/>
                <SelectInput id="fail-safe-result" name="Fail safe" bind:value={results.failSafe.value}/>
            </div>
        </div>
    </div>
    <div class="pt-5">
