...
<tr>
    ...
    {#if work}
        ...
        {:else if notResolved(work) }
            ...
            {:else }
            <td class="relative whitespace-nowrap p-2 text-right text-sm font-medium sm:pr-6">-</td>
        {/if}
    {:else }
        <td class="whitespace-nowrap p-2 text-sm text-gray-500 text-center bg-gray-100" colspan="6">Pending</td>
        <td class="relative whitespace-nowrap p-2 text-right text-sm font-medium sm:pr-6">
            <button class="text-gray-600 hover:text-gray-900" on:click={() => startWork(visitId, location.id)}>Start
            </button>
        </td>
    {/if}
</tr>
